		...
		BeginDrawing();
			...
			if(!firstTime)
			{
				...
				if (winnerText)
				{
					...
					DrawText(winnerText, GetScreenWidth() / 2 - textWidth / 2, GetScreenHeight() / 2 - (30 * 3), 60, YELLOW);
				}
			}
			else
			{
				ClearBackground(BLACK);
				int textWidth = MeasureText(menuText, 60);
				DrawText(menuText, GetScreenWidth() / 2 - textWidth / 2, GetScreenHeight() / 2 - 30, 60, YELLOW);
			}
			DrawFPS(10, 10);
		EndDrawing();
	}
